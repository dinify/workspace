...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    case 'FETCH_LOGGEDRESTAURANT_DONE': {
      const actualOptions = action.payload.res.options;
    ...
    case 'COLLAPSE_OPTION_INIT': {
      ...
      return R.over(
        R.lensProp('all'),
        R.map(o => R.assoc('collapsed', o.id === id ? !o.collapsed : false, o)),
      )(state);
    }
    ...
    case 'CREATE_CHOICE_DONE': {
      ...
      return R.assocPath(['all', optionId, 'choices', newChoice.id], newChoice)(
        state,
      );
    }
    ...
    case 'REMOVE_CHOICE_FAIL': {
      ...
      return R.assocPath(['all', optionId, 'choices', id], state.backup[id])(
        state,
      );
    }
