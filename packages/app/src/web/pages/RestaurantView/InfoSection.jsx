...
const InfoSection = ({ classes, restaurant }) => {
  let addr = false;
  ...
  if (
    ...
    displayOpenHours = true;
  const weekdays = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
  const localizedWeekdays = {};
  [1, 2, 3, 4, 5, 6, 7].forEach((key, i) => {
    localizedWeekdays[weekdays[i]] = cldr.Calendars.weekdays({ width: 'wide' })[
      key
    ];
  });
  const shortTimeSkeleton = cldr.Calendars.manager.getCalendarPatterns()
    .timeFormats['short'];
  ...
  const getOpenState = () => {
    ...
    let at = null;
    const getHours = day => {
      const hrs = openHours[weekdays[day]];
      if (!hrs) return [];
      return hrs
        .map(interval => ({
    ...
    const tomorrowHours = getHours((new Date().getDay() + 1) % 7);
    ...
    return [
      ...
      isOpen ? 'hours.closes_at' : 'hours.opens_at',
      { time: at },
    ];
  };
  ...
  return (
    <div>
      ...
      {displayOpenHours && (
        <ExpansionPanel
          classes={{ root: classes.expansionRoot, expanded: classes.nopad }}
        >
            ...
            <table style={{ borderSpacing: 0, paddingLeft: 56 }}>
              <tbody>
                {[0, 1, 2, 3, 4, 5, 6].map(index => {
                  const offsetWeekday =
                    weekdays[(new Date().getDay() + index) % 7];
      ...
      {restaurant.contact && restaurant.contact.phone && (
        <a
          rel="noopener noreferrer"
          target="_blank"
      ...
      {restaurant.contact && restaurant.contact.email && (
        <a
          rel="noopener noreferrer"
          target="_blank"
      ...
      {restaurant.contact && restaurant.contact.website && (
        <a
          rel="noopener noreferrer"
          target="_blank"
